...
\section{Probleme}\label{chap:background-sec:trace}
...
Man betrachte zuerst die Erkennung von eines zyklischen Locking. 
...
geschlossen worden ist, währen das Lock $m_2$ schließt~\cite{lock-tree}.
...
false-positives verhinder werden können, wird in dem Kapitel zur Implementierung
...
Es ist also möglich solche Situationen zu erkennen. Um solche Situationen verhindern 
...
Situationen, bei denen eine gesendet Nachricht auf einem Channel 
nie ausgelesen wird lassen sich allerdings aus dem Trace direkt erkennen.
...
In dem Beispiel in Abb.~\ref{Chap:Analyze-Sec:Channel-SubSec:Dangling-Fig:NoSync}
...
Wir betrachten den Buffer eines Channels als FIFO-queue. 
...
wird nun für gepufferte Channels überprüft, ob die Differenz zwischen der 
Anzahl der gesendeten 
Nachrichten in dem Send-Statement und der Anzahl der empfangenen 
Nachrichten in dem Receive-Statement kleiner als die maximale Kapazität
des Channels ist. 
