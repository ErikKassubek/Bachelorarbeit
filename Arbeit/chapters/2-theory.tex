...
ausgeführt werden ($\#s_{i, \not\gtreqless}$). Das selbe wird auch für jedes Receive-Statements $r_i$
bestimmt ($\#r_{i, <}, \#r_{i, \not\gtreqless}$). Damit ein Send-Receive Paar $s_i - r_j$ auf 
...
\begin{align}
  \#s_{i, <} + \#s_{i, \not\gtreqless} &\geq \#r_{j, <} + \#r_{j, \not\gtreqless} \label{Form:1}\\
