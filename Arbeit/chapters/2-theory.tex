...
\begin{itemize}
  \item Lock: Die Lock-Operation wird vor dem Eintritt in einen kritischen 
    Bereich aufgerufen. Sie versucht den Mutex zu schließen. Wird es im moment
...
Theoretisch verhalten sich Mutexe in Go wie Semaphore, d.h.~es ist möglich, 
...
soll, von der es beansprucht wurde. Für die Erkennung von 
Problemen wird daher angenommen, das dies immer der Fall ist.
...
dass heißt, es wird bei einem Receive immer die ältest in dem
Buffer vorhandene Nachricht ausgegeben. Für die Implementierung des 
Detektors wird daher von diesem Verhalten ausgegangen.\\\\
