...
Um solche Situationen zu erkennen werden alle möglichen Paaren von Kommunikationspartner,
...
\begin{figure}[h]
  \centering
  \lstinputlisting[frame=none, numbers=none, xleftmargin=0.35\textwidth, xrightmargin=0.35\textwidth]{code/vector-clock.txt}
\end{figure}\\
...
$s_{1, pre} \not\lessgtr r_{pre}$. Sie kann also gleichzeitig mit der 
...
ausgeführt werden ($\#s_{i, \not\lessgtr}$). Das selbe wird auch für jedes Receive-Statements $r_i$
bestimmt ($\#r_{i, <}, \#r_{i, \not\lessgtr}$). Damit ein Send-Receive Paar $s_i - r_j$ auf 
...
\begin{align}
  \#s_{i, <} + \#s_{i, \not\lessgtr} &\geq \#r_{j, <} + \#r_{j, \not\lessgtr} \label{Form:1}\\
  \#s_{i, <} & \leq \#r_{j, <} \label{Form:2}
\end{align}
